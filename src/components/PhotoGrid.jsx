        ...
        return (
          <figure key={p.id} className="card" style={{ margin: 0, background: C.white, borderRadius: 12, overflow: "hidden", border: "1px solid rgba(28,38,64,.08)", opacity: dimUnkept && !p.kept ? 0.55 : 1 }}>
            <div style={{ position: "relative" }}>
              ...
              {showEdited && (
                <span style={{ position: "absolute", top: 8, left: 8, background: C.ink, color: C.gold, fontSize: 10, fontWeight: 700, padding: "2px 7px", borderRadius: 999, letterSpacing: 0.5 }}>EDITED</span>
              )}
              {!p.kept && (
                <span style={{ position: "absolute", bottom: 8, left: 8, background: "rgba(28,38,64,.85)", color: C.bg, fontSize: 10, padding: "2px 7px", borderRadius: 4 }}>below threshold</span>
              )}
            </div>
            <figcaption style={{ padding: "8px 10px", fontSize: 11, color: C.second, display: "flex", justifyContent: "space-between" }}>
              <span>focus {p.focus}</span><span>exp {p.exposure}</span>
            </figcaption>
          </figure>
        );
      })}
